...
export async function createProduct(req, res) {
  try {
    ...
    let parsedSizes = parseJSONField(sizesString, []);
...
export async function updateProduct(req, res) {
  try {
    ...
    let parsedSizes = parseJSONField(sizesString, existingProduct.sizes);
    ...
    let updatedImages = existingProduct.images.filter(
      (img) => !parsedRemovedImages.includes(img.url),
    );
    ...
    if (isActive !== undefined)
      existingProduct.isActive = isActive === "true" || isActive === true;
    if (isFeatured !== undefined)
      existingProduct.isFeatured = isFeatured === "true" || isFeatured === true;
...
export async function toggleProductStatus(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Status toggled successfully", success: true, product });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
}
...
export async function getMyProducts(req, res) {
  try {
    const seller = req.user;
    const products = await ProductModel.find({ seller: seller._id }).sort({
      createdAt: -1,
    });
    res.status(200).json({
